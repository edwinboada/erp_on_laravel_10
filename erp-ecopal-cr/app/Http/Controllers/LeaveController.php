...
//            dd($actionArr);
    ...
    public function jsoncount(Request $request)
    {
        ...
        $expression  = DB::raw('COALESCE(SUM(leaves.total_leave_days),0) AS total_leave');
        $string      = $expression->getValue(DB::connection()->getQueryGrammar());
        foreach ($leave_types as  $type) {
            $counts=Leave::select($string)->where('leave_type_id',$type->id)->groupBy('leaves.leave_type_id')->where('employee_id',$request->employee_id)->first();
